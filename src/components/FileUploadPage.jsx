...
const FileUploadPage = () => {
  ...
  async function ParseData(file) {
    ...
    try {
      ...
      if (expectedKeys.every(key => actualKeys.includes(key))) {
        ...
        toast.success(
          "PDF/DOCX parsed successfully. You can proceed.",
          { duration: 3000, position: "top-right" }
        );
      } else {
        toast.error(
          "Parsed file does not match required format! Please Retry..",
          { duration: 3000, position: "top-right" }
        );
      }
      ...
      console.error(err);
      toast.error(
        "Failed to parse resume! Please Retry..",
        { duration: 3000, position: "top-right" }
      );
    }
  }
  ...
  const handleDocUpload = (event) => {
    ...
    if (!file) return;
    toast.success(
            "Probably backend is waking up. Please wait a moment…",
            { duration: 8000, position: "top-right" }
    );
    if (file.type.includes("pdf") || file.type.includes("word")) {
      ParseData(file); // ✅ async call
    } else {
      toast.error(
